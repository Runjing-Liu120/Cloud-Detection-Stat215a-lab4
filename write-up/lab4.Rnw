...
In image data, neighboring pixels are generally not independent, and there is reason to believe there is some ``continuity" in the structure of an image. In light of this, we attempt to 

\subsection{Logistic regression and LASSO}
As a first attempt to predict the expert labels, we fit a logistic regression on the original eight features as well as the  However, to guard 
