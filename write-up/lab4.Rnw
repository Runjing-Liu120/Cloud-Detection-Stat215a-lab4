...
We present the predictive accuracy of using logistic regression, logistic-LASSO, random forest, and support vector machines, and compare the benefits, if any, of using the smoothed features. Our main results are summarized in table~\ref{tab:acc_results}.
...
As a baseline, we use a simple logistic regression on the original eight features to predict the expert labels. We fit the model using the first and second images, and our predictive accuracy on the third image with the original features is \Sexpr{image3_pred$accuracy}. Using the smoothed features, we get an accuracy of \Sexpr{image3_pred_smo$accuracy}. Therefore, it appears that smoothing the features did not have a significant effect when doing logistic regression.
...
We now examine the accuracy of the LASSO, trained on the first two images with cross validation as described in ~\ref{CV_descr}. The predictive accuracy on the held out third image was \Sexpr{image3_lasso_pred$accuracy} when using the original features, and \Sexpr{image3_lasso_smo_pred$accuracy} when using the smoothed features. Like in the logistic regression, smoothing the features did not appear to make a big difference here.
...
Without feature smoothing, the predictive accuracy on image 3 is \Sexpr{image3_rf_pred$accuracy}. With feature smoothing, the predictive accuracy on  image 3 was \Sexpr{image3_rf_smo_pred$accuracy}. In contrast, The training accuracy on image 2 without smoothing is \Sexpr{image2_rf_pred$accuracy}, and the training accuracy with smoothing is \Sexpr{image2_rf_smo_pred$accuracy}. The huge gap suggests that there is overfitting.

A noticable phenomenon during the training of random forest models is that if we do not perform feature normalizations, then the prediction accuracy on image 3 will change dramatically. It could be due to the fact that we do the scaling on 3 images independently, i.e., we use their own mean and standard deviation to normalize each feature. However, since the assumption that these 3 images have identical distribution could be wrong, after normalizing each feature will be biased and the tree algorithm would not adapt to such changes.
...
Finally, we examine prediction using SVM. As described in section \ref{CV_descr}, the first two images were used as a training, set and 2-fold cross-validation was used to set the cost of contraint violation.

Here, we only fit using 3 features: NDAI, CORR, and DF, variables found to be important by the LASSO (see section~\ref{feature_selection}). Using these tree features, predictive

\begin{table}[h]
 \centering
 \begin{tabular}{lllll}
 & logistic & LASSO & RF & SVM \\
 smoothed features & \Sexpr{image3_pred$accuracy} &
 \Sexpr{image3_lasso_pred$accuracy}& 100\% !! & 100\% !!\\
 original features & \Sexpr{image3_pred_smo$accuracy} &  \Sexpr{image3_lasso_pred$accuracy} & 99\% !! & 99\% !!
 \end{tabular}
...
Random forest models also reports feature importance by giving the Gini coefficient. In our original model which is trained on image 1 and 2 without smoothing, the top 3 most important features are \texttt{NDAI, AF} and \texttt{BF}. If we add the smoothing features, then 3 of the most important features would become \texttt{NDAI\_smoothed}, \texttt{AF\_smoothed} and \texttt{SD\_smoothed}. The figure \ref{fig:rf_importance} shows the Gini coefficient of the model in decreasing order.

\begin{figure}[h]
\centering
<<rf_feature_importance>>=
df <- data.frame(rf_fit_orig$variable.importance)
names(df) <- 'Variable_Importance'
df$Variable <- rownames(df)
ggplot(data=df, aes(x = reorder(Variable, -Variable_Importance), y = Variable_Importance)) +
  geom_bar(position = 'identity', stat = 'identity') +
  labs(x="Variable",y="Variable Importance")
@
\caption{Variable importance of the Random Forest model by GINI coefficient}
